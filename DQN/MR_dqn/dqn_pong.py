...
tensorboard_dir_name = 'Random EPS'
...
epsilon_method = 1
epsilon_range = np.linspace(0.02, 0.8, 256)
...
memory = PrioritizedReplayBuffer(
    mem_size, alpha_per, use_gpu=use_gpu) if use_per else ReplayBuffer(mem_size, use_gpu=use_gpu)

double_dqn = 1
dueling = 1
grad_norm = 10
behavior_model = Net(actions=env.action_space.n,
                     dueling=dueling).to(device=device)
target_model = Net(actions=env.action_space.n,
                   dueling=dueling).to(device=device)
...
for episode in range(episodes):
  ...
  obs = torch.as_tensor(obs, device=device,
                        dtype=torch.float32) if use_gpu else obs
  tot_reward += reward
  ...
  while not done:
    ...
    elif (epsilon_method==1) and (step_counter%epsilon_interval==0):
    ...
    obs_new = torch.as_tensor(obs_new, device=device,
                              dtype=torch.float32) if use_gpu else obs_new
    ...
    if (step_counter > warmup) and (step_counter % train_every == 0):
      ...
      with torch.no_grad():
        if double_dqn:
          ...
          q_value = targets_next.gather(
              1, double_targets.argmax(1).unsqueeze(1)).squeeze()
        else:
          targets_next = target_model(states_next).detach()
          q_value = targets_next.max(dim=1)[0]
      labels = rewards_tensor + \
          (dones_tensor.logical_not_()) * discount * q_value
      predictions = targets.gather(1, actions_tensor.unsqueeze(1)).squeeze()
      losses = loss(predictions, labels)
      if use_per:
        losses.register_hook(
            lambda x: x * torch.as_tensor(weights, device=device, dtype=torch.float32))
